						<ul	 id="da-thumbs" class="da-thumbs">
										<?php $query = mysqli_query($conn,"select * from parent_class
										LEFT JOIN class ON class.class_id = parent_class.class_id
										LEFT JOIN subject ON subject.subject_id = parent_class.subject_id
										where parent_id = '$session_id' and school_year = '$school_year' ")or die(mysqli_error());
										$count = mysqli_num_rows($query);
										
										if ($count > 0){
										while($row = mysqli_fetch_array($query)){
										$id = $row['parent_class_id'];
				
										?>
											<li id="del<?php echo $id; ?>">
												<a href="my_child.php<?php echo '?id='.$id; ?>">
